...
static const char *MemberTagStr(int tag)
{
	switch(tag) {
...
static int vtu_add_entry(struct vtu *entry)
{
	...
	/* Next, we take care of the VLAN map, which is a per-port bitfield
	 * at offset 6
	 */
	...
	while(1) {
		x = 0;
		phy_write(GLOBAL_REGS_1, VTU_OPS_REGISTER,
		  (1 << 15) | VTU_OP_GET_NEXT);
		...
		while(1) {
			...
			/* wait for VB=0 in Global Reg 0x05 */
			if (! (x & (1 << 15)))
			break;
		}
...
L1:
			for(i=0; i < 7; i++) {
				/* don't set "our" bit in "our" mask register */
				if (i != p) {
					switch(i) {
					  case 0:
						if ((r7 & 0x3) != 0x3) {
...
static void usage(char **argv) {
	fprintf(stderr,
	  ...
	  "  -5, --ethwlan           Sets port A on VLAN, other ports to\n"
	  "                            switch mode\n"
	  "  -C, --ethinfo           Retrieves info on the onboard switch\n"
	  "  -Q  --ethbus            MII management bus number\n"
	  "                            (implementation dependent)\n"
	  "  -h, --help              This help\n"
	  "\n"
	  "Additional port config options:\n"
	  "  -p, --port <port>       PHY Port to operate on [0-1]\n"
	  "  -a, --autoneg           Enable auto-negotiation of <port>\n"
	  "                            (default behavior)\n"
...
int main(int argc, char **argv) {
	...
	if ((opt_ethswitch || opt_ethvlan || opt_ethwlan)) {
		// Marvell 88E6020 Network Switch
		if(swmod == 512) {
			...
			/* With this chip, ethvlan and wlan are the same since
			 * there are only have 2 ports
			 */
			if(opt_ethvlan || opt_ethwlan) {
				...
				for(i=0; i < 7; i++) {
					// leave the port alone by default
					entry.tags[i] = 3;
				...
				entry.v = 1;
				entry.vid = 1;
				// CPU port is always egress tagged
				entry.tags[5] = 2;

				// External Port A egress untagged
				entry.tags[0] = 1;
				vtu_add_entry(&entry);

				// Do not configure port A again
				entry.tags[0] = 3;

				// Setup VLAN #2 on P1 and P5
				entry.vid = 2;

				// External PORT B egress untagged
				entry.tags[1] = 1;
				vtu_add_entry(&entry);
			}
		// Marvell 88E6070 Network Switch
		} else if (swmod == 1792) {
			// Used on the TS-8700
			// enable all PHYs
			...
			if(opt_ethvlan) {
				...
				entry.vid = 1;
				// TS-4710 CPU port is always egress tagged
				entry.tags[0] = 2;
				// External Port A egress untagged
				entry.tags[1] = 1;
				vtu_add_entry(&entry);
				...
				entry.tags[2] = 1;
				vtu_add_entry(&entry);
				// Do not configure port B again
				entry.tags[2] = 3;

				// Setup VLAN #2 on P0 and P3
				entry.vid = 3;
				// External PORT C egress untagged
				entry.tags[3] = 1;
				vtu_add_entry(&entry);
				// Do not configure port C again
				entry.tags[3] = 3;

				// Setup VLAN #2 on P0 and P4
				entry.vid = 4;
				// External PORT D egress untagged
				entry.tags[4] = 1;
				vtu_add_entry(&entry);
			} else if (opt_ethwlan) {
				struct vtu entry;
